...
BERTScore 是 2019 年提出的语义评估指标，核心思路是：**不比较词面是否相同，而是比较语义是否相近**。它用 BERT 把每个词编码成一个向量，再用向量之间的余弦相似度来衡量匹配程度。

![[Pasted image 20260509165008.png]]
![[Pasted image 20260509165024.png]]
![[Pasted image 20260509165038.png]]
![[Pasted image 20260509165051.png]]
![[Pasted image 20260509165121.png]]
计算过程分四步：

**第一步：生成上下文向量。** 把参考文本和生成文本分别送入 BERT，每个词得到一个考虑了整句上下文的高维向量。这是 BERTScore 最关键的地方——同一个词在不同语境下的向量是不同的。

**第二步：构建相似度矩阵。** 对生成文本的每个词和参考文本的每个词，两两计算余弦相似度，形成一个矩阵。"feline"和"cat"的向量方向极为接近，相似度高达 0.93；"rested"和"sat"达到 0.85。

**第三步：贪心匹配。** 计算 Precision 时，生成文本的每个词去找参考文本中与之最相似的词；计算 Recall 时反过来，参考文本的每个词去找生成文本中最相似的词。

**第四步：汇总得 F1。** 对所有匹配得分取平均，再算调和平均，得到最终的 BERTScore F1。

---

**BERTScore 的局限：**

- 计算成本高，需要跑 BERT 前向传播，速度比 ROUGE/BLEU 慢很多
- 可解释性弱，得 0.87 是什么意思不如 ROUGE 直观
- 对底层 BERT 模型的选择敏感，不同模型得分不可直接比较
- 语义相近不等于事实正确，"猫在垫子上"和"猫不在垫子上"语义相近，BERTScore 却可能给高分

所以实践中通常把 BERTScore 和 ROUGE 结合着用，两者互补。